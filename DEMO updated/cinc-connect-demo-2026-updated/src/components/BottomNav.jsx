...
import financialHubIconRaw from '../ICONS/Financial Hub.svg?raw'
...
export default function BottomNav() {
  ...
  const { residentViewStack, navStyle } = useMode()
  if (!isBoard && residentViewStack.length > 0) return null
  if (!isBoard) return navStyle === 'v2' ? <ResidentNavV2 /> : <ResidentNav />
  if (['/members-list', '/member-detail'].includes(pathname)) return null
  return (
...
/* ── Resident Nav ────────────────────────────────────── */
const BASE_RESIDENT_ITEMS = [
  { label: 'Feed',          icon: FeedIcon },
  { label: 'My Community',  icon: CommunityIcon },
  { label: 'Financial Hub', icon: FinancialHubIcon },
  { label: 'My Properties', icon: MyPropertyIcon },
  { label: 'More',          icon: MoreIcon },
]

const BOARD_ROOM_ITEM = { label: 'Board Room', icon: BoardRoomIcon }

function useResidentItems() {
  const { showBoardRoom } = useMode()
  if (!showBoardRoom) return BASE_RESIDENT_ITEMS
  return [
    BASE_RESIDENT_ITEMS[0],
    BOARD_ROOM_ITEM,
    ...BASE_RESIDENT_ITEMS.slice(1),
  ]
}

function ResidentNav() {
  const { residentTab, setResidentTab } = useMode()
  const items = useResidentItems()
  return (
    <nav className="resident-nav">
      <div className="resident-nav__bg" />
      {items.map(({ label, icon: Icon }) => {
        const isActive = residentTab === label
        return (
          <button
            key={label}
            className={`resident-nav__item${isActive ? ' resident-nav__item--active' : ''}`}
            onClick={() => setResidentTab(label)}
          >
...
/* ── Resident Nav V2 — floating pill ─────────────────── */
function ResidentNavV2() {
  const { residentTab, setResidentTab } = useMode()
  const items = useResidentItems()
  return (
    <nav className="resident-nav-v2">
      {items.map(({ label, icon: Icon }) => {
        const isActive = residentTab === label
        return (
          <button
            key={label}
            className={`rnv2-item${isActive ? ' rnv2-item--active' : ''}`}
            onClick={() => setResidentTab(label)}
          >
            <span className="rnv2-icon"><Icon /></span>
            <span className="rnv2-label">{label}</span>
          </button>
        )
      })}
    </nav>
...
function ParticipationIcon() {
  // voting.svg — ballot box icon
  return (
    <svg width="22" height="19" viewBox="0 0 65 53.61" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path fill="currentColor" d="M57.79,19.97h-10.44v-6.22c0-3.98-3.24-7.22-7.22-7.22h-1.14V3.61c0-1.99-1.62-3.61-3.61-3.61h-6.76c-1.99,0-3.61,1.62-3.61,3.61v2.92h-1.14c-3.98,0-7.22,3.24-7.22,7.22v6.22H6.21C2.79,19.97,0,22.76,0,26.18v20.22c0,3.42,2.79,6.21,6.21,6.21h51.58c3.42,0,6.21-2.79,6.21-6.21V26.18c0-3.42-2.79-6.21-6.21-6.21ZM29.01,3.61c0-.22.18-.4.4-.4h6.76c.22,0,.4.18.4.4v2.92h-7.56V3.61ZM19.65,13.75c0-2.21,1.8-4.01,4.01-4.01h16.68c2.21,0,4.01,1.8,4.01,4.01v6.22H19.65v-6.22ZM60.79,46.4c0,1.65-1.35,3-3,3H6.21c-1.65,0-3-1.35-3-3V26.18c0-1.65,1.35-3,3-3h51.58c1.65,0,3,1.35,3,3v20.22Z"/>
      <path fill="currentColor" d="M39.18,30.47l-8.56,8.56-3.81-3.81c-.63-.63-1.65-.63-2.27,0-.63.63-.63,1.65,0,2.27l4.94,4.94c.31.31.72.47,1.14.47s.83-.16,1.14-.47l9.7-9.7c.63-.63.63-1.65,0-2.27-.63-.63-1.65-.63-2.27,0Z"/>
    </svg>
  )
}
...
function BoardRoomIcon() {
  const svg = boardRoomIconRaw
    .replace('<svg ', '<svg fill="currentColor" width="26" height="22" ')
  return <span style={{display:'flex',alignItems:'center',justifyContent:'center',width:26,height:22}} dangerouslySetInnerHTML={{__html: svg}} />
}

function MyPropertyIcon() {
  const svg = myPropertyIconRaw
    .replace(/fill="#FFF8EA"/g, 'fill="currentColor"')
    .replace(/width="71"/, 'width="22"')
    .replace(/height="76"/, 'height="24"')
  return <span style={{display:'flex',alignItems:'center',justifyContent:'center',width:24,height:24}} dangerouslySetInnerHTML={{__html: svg}} />
}

function CommunityIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 73.45 76.22" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path fill="currentColor" d="M14.82,71.22H5V11.17c0-3.4,2.59-6.17,5.77-6.17h40.37c3.18,0,5.77,2.77,5.77,6.17v17.36c0,1.38,1.12,2.5,2.5,2.5s2.5-1.12,2.5-2.5V11.17c0-6.16-4.83-11.17-10.77-11.17H10.77C4.83,0,0,5.01,0,11.17v62.55c0,1.38,1.12,2.5,2.5,2.5h12.32c1.38,0,2.5-1.12,2.5-2.5s-1.12-2.5-2.5-2.5Z"/>
...
function FinancialHubIcon() {
  return <span style={{display:'flex',alignItems:'center',justifyContent:'center',width:24,height:24}} dangerouslySetInnerHTML={{__html: financialHubIconRaw.replace(/fill="#FFF8EA"/g, 'fill="currentColor"')}} />
}
