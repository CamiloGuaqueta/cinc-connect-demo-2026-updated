...
function BoardFeed() {
  ...
  return (
    <div className="screen feed-screen" ref={screenRef}>
...
const RESIDENT_POSTS = [
  ...
  },
  {
    ...
    isBoardMember: false, subtitle: 'Senior Real Estate Agent · Cardinal Realty Group',
    title: 'Cardinal Hills Spring Market Report',
    body: 'Buyer demand has surged over the past 90 days and the data is telling a story every homeowner should understand. ..',
    image: soldPhoto, likes: 31, comments: 7, avatarImg: SARAH_PHOTO,
...
const NMI_MOCK = {
  ...
  avm:    { value: 425000, change: 12000, rangeLow: 398000, rangeHigh: 452000 },
  score:  { composite: 72, label: 'Good' },
  market: {
    medianSalePrice: 418000, medianSalePricePctChange: 4.2,
    homesSold90d: 23, avgDaysOnMarket: 18,
    activeListings: 7, marketCondition: "Seller's Market",
  },
  areaProfile: {
    ownerOccupiedPct: 78, medianHomeValue: 412000,
    schoolDistrict: 'Orange County Schools',
  },
  recentSales: [
    { address: '101 Maple Ridge Dr', beds: 3, baths: 2, price: 415000, date: 'Apr 12' },
    { address: '234 Birchwood Ct',   beds: 4, baths: 3, price: 498000, date: 'Apr 8'  },
  ],
  blog: {
    category: 'Market Update',
    title: 'Cardinal Hills Spring Market Report',
    excerpt: 'Buyer demand has surged over the past 90 days. Here\'s what the data says about your home\'s value this season.',
  },
  agent: {
    name: 'Sarah Mitchell', title: 'Senior Real Estate Agent',
    brokerage: 'Cardinal Realty Group', phone: '(407) 555-0192',
  },
}

const NMI_VARIANT_LABELS = [
  'Market Overview', 'Home Valuation', 'Market Score',
  'Homes Sold', 'Median Sale Price', 'Active Listings',
  'Area Profile', 'Recent Sales', 'Neighborhood Insights', 'Your Listing Agent',
]

function NmiPreviewCard({ onViewMore }) {
  ...
  function nextVariant(e) {
    e.stopPropagation()
    setVariant(v => (v + 1) % NMI_VARIANT_LABELS.length)
  }

  return (
    <div className="nmi-preview-card card">
      {/* Header */}
      <div className="nmi-preview-card__header">
        <div className="nmi-preview-card__header-left">
          {/* NMI icon badge — tap to cycle variants */}
          <div className="nmi-preview-card__badge" onClick={nextVariant}>
            <svg width="22" height="22" viewBox="0 0 71.12 73.93" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div>
            <p className="nmi-preview-card__name">Home Value Insights</p>
            <p className="nmi-preview-card__subtitle">{d.neighborhood} · {NMI_VARIANT_LABELS[variant]}</p>
          </div>
        </div>
        <span className="nmi-preview-card__date">Apr 2026</span>
      </div>

      {/* Body — variant-specific */}
      <div className="nmi-preview-card__body">

        {/* 0 — KPI Grid */}
        {variant === 0 && (
          <div className="nmi-pv-kpi-grid">
            {[
              { label: 'Est. Home Value', value: $K(d.avm.value),            delta: `▲ ${$K(d.avm.change)} / yr`,                    up: true },
              { label: 'Median Sale',     value: $K(d.market.medianSalePrice), delta: `▲ ${d.market.medianSalePricePctChange}% YoY`, up: true },
            ].map(({ label, value, delta, up }) => (
              <div key={label} className="nmi-pv-kpi">
                <p className="nmi-pv-kpi__label">{label}</p>
                <p className="nmi-pv-kpi__value">{value}</p>
                <p className={`nmi-pv-kpi__delta${up ? ' up' : ''}`}>{delta}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {/* 1 — AVM */}
        {variant === 1 && (
          <div className="nmi-pv-avm">
            <p className="nmi-pv-avm__eyebrow">Estimated Home Value</p>
            <p className="nmi-pv-avm__value">{$full(d.avm.value)}</p>
            <span className="nmi-pv-avm__conf-pill">Very High Confidence · 85%</span>
          </div>
        )}

        {/* 2 — Market Score */}
        {variant === 2 && (
          <div className="nmi-pv-score">
            <div className="nmi-pv-score__top">
              <div className="nmi-pv-score__big">
                <span className="nmi-pv-score__num">{d.score.composite}</span>
                <span className="nmi-pv-score__out">/100</span>
              </div>
              <div>
                <span className="nmi-pv-score__badge">{d.score.label}</span>
                <p className="nmi-pv-score__county">Top 28% in County</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* 3 — Homes Sold */}
        {variant === 3 && (
          <div className="nmi-pv-hero">
            <p className="nmi-pv-hero__num">{d.market.homesSold90d}</p>
            <div className="nmi-pv-hero__right">
              <p className="nmi-pv-hero__label">Homes sold in the past 90 days</p>
              <p className="nmi-pv-hero__sub">Avg. {d.market.avgDaysOnMarket} days on market</p>
            </div>
          </div>
        )}
        ...
        {variant === 4 && (
          <div className="nmi-pv-hero">
            <p className="nmi-pv-hero__num">{$K(d.market.medianSalePrice)}</p>
            <div className="nmi-pv-hero__right">
              <p className="nmi-pv-hero__label">Median Sale Price</p>
              <p className="nmi-pv-hero__sub up">▲ {d.market.medianSalePricePctChange}% year over year</p>
            </div>
          </div>
        )}
        ...
        {variant === 5 && (
          <div className="nmi-pv-hero">
            <p className="nmi-pv-hero__num">{d.market.activeListings}</p>
            <div className="nmi-pv-hero__right">
              <p className="nmi-pv-hero__label">Active Listings</p>
              <span className="nmi-pv-hero__badge">{d.market.marketCondition}</span>
            </div>
          </div>
        )}
        ...
        {/* 6 — Area Profile */}
        {variant === 6 && (
          <div className="nmi-pv-profile">
            <div className="nmi-pv-profile__item">
              <p className="nmi-pv-profile__val">{d.areaProfile.ownerOccupiedPct}%</p>
              <p className="nmi-pv-profile__label">Owner Occupied</p>
            </div>
            <div className="nmi-pv-profile__item">
              <p className="nmi-pv-profile__val">{$K(d.areaProfile.medianHomeValue)}</p>
              <p className="nmi-pv-profile__label">Median Home Value</p>
            </div>
            <div className="nmi-pv-profile__item">
              <p className="nmi-pv-profile__val nmi-pv-profile__val--sm">{d.areaProfile.schoolDistrict}</p>
              <p className="nmi-pv-profile__label">School District</p>
            </div>
          </div>
        )}
        ...
        {/* 7 — Recent Sales */}
        {variant === 7 && (
          <div className="nmi-pv-sales">
            {d.recentSales.map((s, i) => (
              <div key={i} className="nmi-pv-sale-row">
                <div className="nmi-pv-sale-row__left">
                  <p className="nmi-pv-sale-row__addr">{s.address}</p>
                  <p className="nmi-pv-sale-row__meta">{s.beds}bd · {s.baths}ba</p>
                </div>
                <div className="nmi-pv-sale-row__right">
                  <p className="nmi-pv-sale-row__price">{$K(s.price)}</p>
                  <p className="nmi-pv-sale-row__date">{s.date}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {variant === 8 && (
          <div className="nmi-pv-blog">
            <span className="nmi-pv-blog__cat">{d.blog.category}</span>
            <p className="nmi-pv-blog__title">{d.blog.title}</p>
            <p className="nmi-pv-blog__excerpt">{d.blog.excerpt}</p>
          </div>
        )}

        {/* 9 — Listing Agent */}
        {variant === 9 && (
          <div className="nmi-pv-agent">
            <img src={SARAH_PHOTO} alt={d.agent.name} className="nmi-pv-agent__photo" />
            <div className="nmi-pv-agent__info">
              <p className="nmi-pv-agent__name">{d.agent.name}</p>
              <p className="nmi-pv-agent__title">{d.agent.title}</p>
              <p className="nmi-pv-agent__brokerage">{d.agent.brokerage}</p>
            </div>
            <div className="nmi-pv-agent__phone">{d.agent.phone}</div>
          </div>
        )}
      ...
      {/* Footer CTA */}
      <button className="nmi-preview-card__cta" onClick={onViewMore}>
        VIEW FULL REPORT
      </button>
    </div>
  )
...
const ACCT_UNITS = [
  ...
  { id: 3, address: '200 Cardinal Hills Dr, Unit 3', acct: 'CH:9902', balance: '$0.00',   autopay: '03/15/2026' },
  { id: 4, address: '400 Cardinal Point Rd, Unit 2', acct: 'CH:4417', balance: '$125.50', autopay: null         },
]
              ...
              <div className="acct-card">
                <div className="acct-card__top">
                  <div className="acct-card__acct-row">
                    <span className="acct-card__acct-label"><strong>Acct:</strong> {unit.acct}</span>
                    <ChevronRightSmallIcon />
                  </div>
                  <p className="acct-card__address">{unit.address}</p>
                  <div className="acct-card__balance-row">
                    ...
                    <span className="acct-card__balance-amount">{unit.balance}</span>
                  </div>
                </div>
                <div className="acct-card__divider" />
                <div className="acct-card__autopay-row">
                  <AutopayIcon />
                  <span className="acct-card__autopay-text">
                    {unit.autopay ? `Autopay scheduled for ${unit.autopay}` : 'No autopay set up'}
                  </span>
                  <ChevronRightSmallIcon />
                </div>
    ...
    <div className="post-card card">
      <div className="post-card__header">
        ...
        <div className="post-card__meta">
          ...
          {post.subtitle && <span className="post-card__subtitle">{post.subtitle}</span>}
      ...
      {onCta && (
        <div className="post-card__cta-wrap">
          <button className="post-card__cta-btn" onClick={onCta}>{post.ctaLabel}</button>
        </div>
      )}
